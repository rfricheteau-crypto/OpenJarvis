...
async def health() -> dict[str, object]:
    ...
    return {
        'ok': True,
        'phase': 'phase-2-webrtc-runtime',
        'variant': 'v2-experimental',
...
async def proto_config() -> dict[str, object]:
    return {
        ...
        'notes': [
            ...
            'V2 is isolated from V1 and runs on a separate port.',
        ],
    }
...
async def proto_hermes_chat_passthrough() -> JSONResponse:
    return JSONResponse(
        {
            'ok': False,
            'detail': 'Use the WebRTC flow at /web/ for V2 runtime testing.',
        },
        status_code=501,
    )
